...
public class SchemaFactoryBean implements FactoryBean<MessageFactory> {
  ...
  /** @return the messageSet object, with its state fields assigned from the
  ...
  public MessageFactory getObject() throws Exception {
    ...
    if (autogen != null) {
      schema.setAutoGeneratorFactory(new AutoGeneratorFactory(autogen));
    }
  ...
  private void addFormatter(final FormatterSpec formatter) throws InstantiationException, IllegalAccessException {
